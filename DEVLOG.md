...
**Update — sensor-accuracy backbone SHIPPED (fork-independent).** Built the shared read engine ahead of the personal-combat forks, since both boarding intel and exploration reads depend on it and it commits to none of the pending decisions:
- `src/threat/sensor.ts` — one pure, seeded module. `readThreat()` (band + confidence + manifest detail) and `readProbability()` (word band → rough %) both driven by a single 0–5 sensor tier. Higher tier = tighter accuracy AND fuller detail; tier 0 = UNKNOWN. Deterministic in (trueValue, tier, seed), so reads are stable across re-render and save/resume with no stored fuzz. 13 unit tests.
